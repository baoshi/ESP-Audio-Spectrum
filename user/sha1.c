...
void ICACHE_FLASH_ATTR sha1_initHmac(sha1nfo *s, const uint8_t* key, int keyLength) {
	uint8_t i;
	os_memset(s->keyBuffer, 0, BLOCK_LENGTH);
	if (keyLength > BLOCK_LENGTH) {
		// Hash long keys
		sha1_init(s);
		for (;keyLength--;) sha1_writebyte(s, *key++);
		os_memcpy(s->keyBuffer, sha1_result(s), HASH_LENGTH);
	} else {
		// Block length keys are used as is
		os_memcpy(s->keyBuffer, key, keyLength);
	}
	// Start inner hash
	sha1_init(s);
...
uint8_t* ICACHE_FLASH_ATTR sha1_resultHmac(sha1nfo *s) {
	uint8_t i;
	// Complete inner hash
	os_memcpy(s->innerHash,sha1_result(s),HASH_LENGTH);
